...
%  Original equations for GSD, B, and W by Lassiter (2017).
    ...
    % cam is an array: XL, YL, ZL, t, [unused], a, f
    ...
    % -------------------------------------
    % create 3D point array in object space
    % -------------------------------------
...
%     hold off;
    ...
    % --------------------------------------
    % plotting the mission for visualization
    % --------------------------------------
    ...
    % iterate through cameras to plot each
    for nn = 1:length(cam)
        % scale the exposure stations for plotting
        scale = 500;
        
        % direct rotation matrix to rotate frustra
        R_tilt = makehgtform('xrotate', deg2rad(cam(nn, 4)));
        R_azimuth = makehgtform('zrotate', deg2rad(cam(nn, 6)));
        R = R_azimuth * R_tilt;

        % shorten to "T" for ease of plotting
        T = cam(nn, 1:3);
        ...
        plot_bounds = plot3( ...
            ...
            fo(3, :) + T(3));
        plot_bounds.Color = 'k';
        plot_bounds.LineWidth = 2;

        % footprints
        v = R(1:3, 1:3) * [v1, v2, v3, v4, v1];

        % conditionals for high obliques: check z-orientations
    ...
    hold off;
    assignin("caller", "fig_world", fig_world)
